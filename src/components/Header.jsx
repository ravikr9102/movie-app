...
import LG_ScreenSidebar from "./LG_ScreenSidebar";
import SM_ScreenSidebar from "./SM_ScreenSidebar";
...
function Header(props) {
  ...
  const [toggleMobileAside, setToggleMobileAside] = useState(false);
  ...
  useEffect(() => {
    const fixToggleVisibility = () => {
      if (navRef.current) {
        const width = navRef.current.clientWidth;
        width > 578 && setToggleVisibility(false);
      }
    };
    window.addEventListener("resize", fixToggleVisibility);
    return () => removeEventListener("resize", fixToggleVisibility);
  }, []);
  return (
    <>
      <header
        ref={navRef}
        className={`flex justify-center w-full md:px-16 items-center bg-[#111111] py-2 text-white font-semibold text-sm max-[990px]:justify-start ${
          toggleVisibility && "pt-0"
        }`}
      >
        <svg
          ...
          fill="currentColor"
          role="presentation"
          className={`hidden cursor-pointer mx-3 max-[990px]:block max-[578px]:flex-shrink-0 max-[578px]:${
            toggleVisibility ? "hidden" : undefined
          }`}
          onClick={() => setToggleMobileAside(true)}
        >
        ...
        <svg
          ...
          viewBox="0 0 64 32"
          version="1.1"
          className={`max-w-full h-auto cursor-pointer max-[990px]:mx-3 max-[578px]:flex-shrink-0 max-[578px]:${
            toggleVisibility ? "hidden" : undefined
          }`}
          onClick={() => navigate("/")}
        >
        ...
        <div
          ...
          className="flex items-center gap-1 cursor-pointer hover:bg-slate-800 p-[6px] px-3 mx-2 rounded max-[990px]:hidden"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="24"
        ...
        <div className="flex relative w-[53%] max-[1124px]:w-[40%] max-[990px]:flex-grow">
          <button className="cursor-not-allowed absolute outline-none bottom-0 top-0 left-0 text-black border-r border-stone-300 px-[15px] max-[578px]:hidden">
            All
          </button>
          <div
            className={`w-full max-[578px]:${
              !toggleVisibility ? "hidden" : undefined
            }`}
          >
            <input
              ...
              className={`w-full ${
                toggleVisibility
                  ? "px-3 py-3 rounded-none bg-[#1F1F1F] focus-visible:shadow-none text-white"
                  : "px-14 py-[6px] rounded-[4px]"
              } text-black font-normal outline-none focus-visible:shadow-[inset_0_0_0_2px_#f5c518] transition-shadow`}
            />
            {/* <AnimatePresence>
              {inputSearch && (
                <SearchQuery
          ...
          {!toggleVisibility ? (
            <svg
              ...
              role="presentation"
              id="search"
              className="min-[578px]:absolute right-3 cursor-pointer text-black top-1/2 transform min-[578px]:-translate-y-1/2 max-[578px]:text-white max-[578px]:ml-auto"
            ...
            <svg
              ...
              id="close"
              className={`${
                toggleVisibility ? "absolute right-4" : undefined
              } w-6 h-6 -translate-y-1/2 top-1/2 transform cursor-pointer`}
              onClick={handleInputVisibility}
            >
              <path
                strokeLinecap="round"
        ...
        <div className="flex items-center">
          <div
            ...
            className="cursor-not-allowed hover:bg-slate-800 p-3 mx-2 rounded max-[990px]:hidden"
          >
            <svg
              width="52"
              height="14"
          ...
          <div className="w-[2px] h-8 border border-stone-700 max-[990px]:ml-3 max-[578px]:hidden"></div>
          <div className="flex items-center gap-1 cursor-not-allowed hover:bg-slate-800 p-[6px] px-4 rounded ml-2 max-[990px]:hidden">
            <svg
              width="24"
              height="24"
              xmlns="http://www.w3.org/2000/svg"
          ...
          <div
            className={`cursor-not-allowed whitespace-nowrap hover:bg-slate-800 p-[6px] py-2 rounded px-3 max-[990px]:ml-2 max-[578px]:m-0 max-[578px]:${
              toggleVisibility ? "hidden" : undefined
            }`}
          >
            Sign In
          ...
          <div>
            <div
              className={`flex items-center cursor-not-allowed hover:bg-slate-800 p-[6px] rounded px-3 max-[990px]:mr-3 max-[578px]:${
                toggleVisibility ? "hidden" : undefined
              }`}
            >
              EN
      ...
      <AnimatePresence>
        {toggleAside && (
          <LG_ScreenSidebar
            ...
            setToggleMobileAside={setToggleMobileAside}
          />
        )}
        {toggleMobileAside && (
          <SM_ScreenSidebar
